...
```powershell
python ..\repo_utils\serve_repo.py . --port 8000
```

To intentionally slow the STL downloads and make the queue/progress bar easier to see, add a small delay:

```powershell
python ..\repo_utils\serve_repo.py . --port 8000 --delay-seconds 1.5
```

To rebuild the catalog, run:
```
python ..\repo_utils\create_repo_catalog.py . "Sample Local Repo"
```

The generated `Hash` and `UiHash` values are based on file contents, so changing the mesh updates the catalog hashes.

If `python` is not on your PATH, `py -3 ..\repo_utils\serve_repo.py . --port 8000` works on many Windows setups too.
